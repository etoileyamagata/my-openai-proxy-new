...
class Service {
  ...
  async saveSettings(env,data,revision) {
    ...
    c.need(!['location','postal_code'].some(k=>Object.hasOwn(data,k)),'商品所在地は店舗別の設定に変わりました。画面を再読込してください。');
    if(Object.hasOwn(data,'store_locations')) s.store_locations={...s.store_locations,...c.storeLocations(data.store_locations)};
  ...
  editable(d) {c.need(!['sending','unknown','published'].includes(d.state),'送信済みの出品準備は変更できません。作業中の商品から出品結果を照会してください。','locked');}
  ...
  async verify(d) {
    ...
    d.ebay_account={seller_key:s.seller_key,seller_id:s.seller_id};
    if(['Success','Warning'].includes(result.ack)) {
      d.state='verified';d.verification={...result,id:c.id(),fingerprint,expires_at:c.now()+c.TTL,seller_id:s.seller_id,seller_key:s.seller_key,settings_revision:s.revision,
        settings:{...Object.fromEntries(c.fields.map(f=>[f,s[f]||''])),...c.itemLocation(d,s)}};
    }
  ...
  async dispatch(data,session) {
    ...
    // Keep the old action compatible with tabs opened before this release.
    if(action==='worklist' || action==='history') return {drafts:await this.store.worklist()};
