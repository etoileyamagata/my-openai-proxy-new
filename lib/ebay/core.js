...
const fields = ['location','postal_code','shipping_policy_id','return_policy_id','payment_policy_id'];
...
function listingXml(call,draft,s) {
  ...
  const location=clean(s.location,'商品所在地',80), postal=clean(s.postal_code,'郵便番号',10);
  need(/^\d{3}-?\d{4}$/.test(postal),'日本の郵便番号を指定してください。');
...
module.exports={AppError,need,id,hash,now,clean,environment,environments,product,imageUrl,listingXml,parse,request,tag,fingerprint,publicSettings,fields,TTL,MAX_IMAGE};
