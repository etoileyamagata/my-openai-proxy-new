...
function responseMessageText(value) {
  ...
  // Decode the XML envelope and any entities in the enclosed HTML.
  for(let i=0;i<2;i++) text=text.replace(/&(#x[0-9a-f]+|#\d+|amp|lt|gt|quot|apos|nbsp);/gi,(entity,key)=>{
  ...
  return text.replace(/<!--[\s\S]*?-->/g,' ')
    ...
    .replace(/<\/?[a-z][^>]*>/gi,' ').replace(/\s+/g,' ').trim().slice(0,12000);
}
function parse(raw,call) {
  ...
  const parsed=new XMLParser({ignoreAttributes:false,removeNSPrefix:true,parseTagValue:false,parseAttributeValue:false,stopNodes:['*.Message']}).parse(raw);
  ...
  return {root,result:{ack:root.Ack,messages:array(root.Errors).map(e=>({code:e.ErrorCode||'',severity:e.SeverityCode||'',classification:e.ErrorClassification||'',message:String(e.LongMessage||e.ShortMessage||'').slice(0,1800)})),
    details:[...new Set(array(root.Message).map(responseMessageText).filter(Boolean))].slice(0,10),
