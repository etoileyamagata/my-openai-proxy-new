...
class Store {
  ...
  async saveSettings(env,s,revision,expectedCredentials) {
    ...
    const r=await this.db.query("UPDATE ail_ebay_settings SET data=$1,revision=revision+1 WHERE environment=$2 AND revision=$3 AND (NOT $4::boolean OR data->>'credentials' IS NOT DISTINCT FROM $5::text) RETURNING data,revision",[JSON.stringify(data),environment(env),revision,arguments.length>=4,expectedCredentials||null]);
  ...
  async reserve(d,s) {
    ...
    return this.transaction(async c=>{
      ...
      if(!reserved.rows.length) throw new AppError('このSKUは送信済み、または結果確認中です。出品履歴を確認してください。',409,'duplicate');
  ...
  async history() {
    const r=await this.db.query(`SELECT data->>'id' id,data->>'environment' environment,data->>'store' store,data->>'state' state,
      ...
      FROM ail_ebay_drafts ORDER BY updated_at DESC LIMIT 100`);
    return r.rows;
  }
